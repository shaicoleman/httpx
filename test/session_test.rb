...
class SessionTest < Minitest::Test
  ...
  def test_session_timeouts_total_timeout
    # Test total_timeout during response phase (similar to request_timeout)
    uri = build_uri("/drip?numbytes=10&duration=4&delay=2&code=200")
    session = HTTPX.with(timeout: { total_timeout: 3, operation_timeout: 10 })
    response = session.get(uri)
  ...
  def test_session_timeouts_total_timeout_connect_phase
    # Test total_timeout that fires during connection phase
    i = 3
    begin
      server = TCPServer.new("127.0.0.1", CONNECT_TIMEOUT_PORT)
    rescue Errno::EADDRINUSE
    ...
    begin
      uri = build_uri("/", origin("127.0.0.1:#{CONNECT_TIMEOUT_PORT}"))
      # Use total_timeout with a longer connect_timeout to ensure total_timeout fires first
      session = HTTPX.with(timeout: { total_timeout: 0.5, connect_timeout: 10 })
      response = session.get(uri)
      verify_error_response(response, HTTPX::TotalTimeoutError)
    ensure
      server.close
